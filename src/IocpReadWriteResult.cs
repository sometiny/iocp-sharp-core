...
namespace IocpSharp
{
    ...
    /// </summary>
    internal class IocpReadWriteResult : AsyncResult
    {
        ...
        public IocpReadWriteResult(AsyncCallback callback, object state) : this(callback, state, null, 0, 0)
        {
        }
        /// <summary>
        /// 构造函数
        ...
        public IocpReadWriteResult(AsyncCallback callback, object state, byte[] buffer, int offset, int count)
            : base(callback, state)
        {
            Buffer = buffer;
            Offset = offset;
